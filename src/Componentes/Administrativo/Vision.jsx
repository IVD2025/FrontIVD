...
const Vision = () => {
  const [vision, setVision] = useState({ titulo: '', contenido: '' });
  ...
  const fetchVisiones = async () => {
    ...
    try {
      ...
      setError('Error al cargar las visiones. Verifica tu conexión o el servidor.');
      console.error('Error al obtener visiones:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!vision.titulo.trim() || !vision.contenido.trim()) {
      MySwal.fire({
        title: '¡Error!',
    ...
    setLoading(true);
    try {
      const payload = { titulo: vision.titulo.trim(), contenido: vision.contenido.trim() };
      let response;
      ...
      const errorMessage = err.response?.data?.message || 'No se pudo guardar la visión. Intenta de nuevo.';
      MySwal.fire({
        title: '¡Error!',
        text: errorMessage,
        icon: 'error', // Corregido de 'success' a 'error'
        confirmButtonText: 'Aceptar',
        confirmButtonColor: '#800020',
      });
      console.error('Error al guardar visión:', err.response ? err.response.data : err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        console.error('Error al eliminar visión:', err.response ? err.response.data : err.message);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleEdit = (vision) => {
    setVision({ titulo: vision.titulo, contenido: vision.contenido });
    setEditingId(vision._id.toString());
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.flexContainer}>
        <section style={styles.gestionTerminosContainer}>
          <h2 style={styles.subtitle}>Gestionar Visión</h2>
          <form onSubmit={handleSubmit} style={styles.form}>
            <div style={styles.formGrid}>
              ...
              <div style={styles.inputGroup}>
                ...
                <textarea
                  name="contenido"
                  placeholder="Contenido de la visión (usa saltos de línea para separar puntos)"
                ...
                />
                <span style={styles.charCount}>{vision.contenido.length}/2000</span>
              </div>
            </div>
        ...
        <section style={styles.terminosGuardadosContainer}>
          ...
          {visiones.length > 0 && (
            <table style={styles.table}>
              ...
              <tbody>
                {visiones.map((vision) => (
                  <tr key={vision._id} style={styles.tr}>
                    ...
                    <td style={styles.td}>{vision.updatedAt ? vision.updatedAt.toLocaleString() : 'N/A'}</td>
                    <td style={styles.td}>
                      <div style={styles.buttonGroup}>
                        <button
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(vision._id.toString())}
                          disabled={loading}
                        >
                          Eliminar
                        </button>
...
// Estilos (sin cambios, copiados de Terminos)
const styles = {
  container: {
    maxWidth: '1100px',
    margin: '30px auto',
